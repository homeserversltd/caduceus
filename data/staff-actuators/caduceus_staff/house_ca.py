...
def _receipt(primitive: str, *, changed: bool, dry_run: bool = False, ok: bool = True, **fields: Any) -> dict[str, Any]:
    return {
        ...
        "firstMissingSignal": "none" if ok else fields.pop("firstMissingSignal", "caduceus-house-ca-refused"),
        **fields,
    }
...
def ensure_root(*, dry_run: bool = False, renewal_authority: str | None = None) -> dict[str, Any]:
    ...
    if exists and not renewal_authority:
        raise RuntimeError("caduceus-house-ca-ring-replacement-refused")
    if dry_run:
...
def issue_leaf(identity: str = "home.arpa", dns_names: Sequence[str] = (), ip_addresses: Sequence[str] = (), *, dry_run: bool = False) -> dict[str, Any]:
    ...
    if bundle_before != bundle_after:
        raise RuntimeError("caduceus-cert-bundle-changed-by-leaf")
...
def _assert_ca_only(path: Path, encoding: str = "pem") -> str:
    ...
    if "CA:TRUE" not in text:
        raise ValueError("caduceus-cert-bundle-not-ca")
...
def bundle_export(platform: str = "linux", *, dry_run: bool = False) -> dict[str, Any]:
    ...
    if exported != fingerprint:
        raise RuntimeError("caduceus-cert-bundle-fingerprint-mismatch")
    out.chmod(0o644)
...
def _expected_root_fingerprint() -> str:
    ...
    if path.is_file():
        ...
        if isinstance(value, str) and value:
            return value
    raise RuntimeError("caduceus-house-ca-root-fingerprint-missing")
...
def trust_install(bundle: str, platform: str = "linux", *, dry_run: bool = False) -> dict[str, Any]:
    ...
    if supplied != expected:
        raise ValueError("caduceus-cert-bundle-fingerprint-mismatch")
...
def apply_nginx(portal: str, upstream: str, certificate: str, key_path: str, *, dry_run: bool = False) -> dict[str, Any]:
    if not portal or not upstream.startswith(("http://", "https://")):
        raise ValueError("caduceus-nginx-input-invalid")
...
def constituent_lock(portal: str, lan_ip: str, *, dry_run: bool = False) -> dict[str, Any]:
    ...
    if not portal:
        raise ValueError("caduceus-constituent-portal-invalid")
...
def _validated_state(transition: dict[str, Any], old: dict[str, Any]) -> dict[str, Any]:
    ...
    if profile != "homeserver" and (portals or constituents):
        raise ValueError("caduceus-state-role-inventory-refused")
    if not isinstance(portals, list) or not isinstance(constituents, list):
        raise ValueError("caduceus-state-shape-invalid")
    return {
        "profile": profile,
...
def state_commit(transition: dict[str, Any], *, dry_run: bool = False) -> dict[str, Any]:
    if not isinstance(transition, dict):
        raise ValueError("caduceus-state-transition-invalid")
    path = state_path()
    ...
    if path.is_file():
        try:
            ...
            raise ValueError("caduceus-state-invalid") from error
...
def portal_admit(portal: str, lan_ip: str, upstream: str, aliases: Sequence[str] = (), *, dry_run: bool = False) -> dict[str, Any]:
    if _profile() != "homeserver":
        return _refusal("portal_admit", "caduceus-portal-admit-profile-refused", portal=portal, failed_child="profile-gate")
    ...
    try:
        ...
        return _refusal("portal_admit", "caduceus-portal-child-failed", portal=portal, failed_child=failed, held_generation=_generation(), children=children)
...
def _emit(call) -> int:
    try:
        ...
        value = _refusal("error", "caduceus-house-ca-refused")
...
def _json_stdin() -> dict[str, Any]:
    try:
        ...
        raise ValueError("caduceus-house-ca-request-invalid") from error
    if not isinstance(value, dict):
        raise ValueError("caduceus-house-ca-request-invalid")
    return value


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="caduceus-house-ca")
