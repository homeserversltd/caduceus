...
{
    let body = json!({});
    match crate::shared::policy::allows_command("games sync") {
        Ok(true) => crate::gate::snake::crossing_path("games/sync", &body)
            .map(|value| (crate::gate::mutation_status(&value), Json(value)))
            .map_err(|signal| staff_refusal("games sync", signal)),
        Ok(false) => Err(crate::gate::api_error("games sync")),
...
pub fn register(router: Router) -> Router {
    router.route("/api/v1/games/sync", axum::routing::post(sync))
}
