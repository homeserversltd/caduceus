...
fn bad_request() -> (StatusCode, Json<crate::gate::ApiErrorBody>) {
    (
        ...
        Json(crate::gate::ApiErrorBody {
            schema: "caduceus.api.error.v1",
            ok: false,
            command: "gaming provider-keys save".to_string(),
...
fn public_keys(value: &Value) -> Value {
    ...
    for (provider, env_key) in PROVIDERS {
        keys.insert((*provider).into(), json!({"configured": configured(source.and_then(|object| object.get(*env_key))), "envKey": env_key}));
    }
