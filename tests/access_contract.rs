...
fn p256_jwk() -> serde_json::Value {
    serde_json::json!({
        "kty": "EC",
        "crv": "P-256",
        "x": "fixture-p256-x-coordinate",
        "y": "fixture-p256-y-coordinate",
    })
}

fn valid_challenge_context(purpose: &str) -> serde_json::Value {
    match purpose {
        "session.mint" => serde_json::json!({"document_public_key": p256_jwk()}),
        "session.prove" => {
            serde_json::json!({"ticket": TICKET, "method": "GET", "target": "/api/v1/access/session"})
        }
        "session.clear" => serde_json::json!({"ticket": TICKET}),
        "capability.mint" => serde_json::json!({
            "ticket": TICKET,
            "action": "profile.module.toggle",
            "target": "homeserver.profile",
        }),
        "pin.change" => serde_json::json!({
            "ticket": TICKET,
            "action": "global.admin.pin.rotate",
            "target": "global.admin.pin",
        }),
        _ => panic!("unknown fixture purpose {purpose}"),
    }
}
...
async fn direct_success_allowlists_only_matching_attendance_material() {
    ...
    let paths = [
        (
            ...
            "challenge.mint",
            serde_json::json!({"purpose":"session.mint","context":valid_challenge_context("session.mint")}),
        ),
        (
...
#[tokio::test(flavor = "current_thread")]
async fn all_purpose_shaped_challenge_contexts_cross_before_staff() {
    let _environment = environment_lock();
    let purposes = [
        "session.mint",
        "session.prove",
    ...
    let cases: Vec<_> = purposes
        .into_iter()
        .map(|purpose| {
            (
                purpose,
                serde_json::json!({"purpose": purpose, "context": valid_challenge_context(purpose)}),
            )
        })
        .collect();
    let expected = cases.clone();
    let path = socket("challenge-contexts");
    let listener = UnixListener::bind(&path).unwrap();
    let worker = thread::spawn(move || {
        for (purpose, context) in expected {
            let (peer, _) = listener.accept().unwrap();
            let mut line = String::new();
            BufReader::new(peer.try_clone().unwrap())
                .read_line(&mut line)
                .unwrap();
            let observed = serde_json::from_str::<serde_json::Value>(&line).unwrap();
            assert_eq!(observed["op"], "challenge.mint");
            assert_eq!(observed["purpose"], purpose);
            assert_eq!(observed["context"], context["context"]);
            let mut peer = peer;
            peer.write_all(success_response("challenge.mint").to_string().as_bytes())
                .unwrap();
            peer.write_all(b"\n").unwrap();
        }
    });
    ...
    for (_, body) in cases {
        let mut local = request("127.0.0.1:40231", Body::from(body.to_string()));
        *local.uri_mut() = "/api/v1/access/challenges/mint".parse().unwrap();
        let response = serve::router().oneshot(local).await.unwrap();
...
async fn malformed_proofs_and_unknown_challenge_purpose_refuse_before_staff() {
    ...
    let cases = [
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"document-attendance","context":{}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"session.mint","context":{}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"session.mint","context":{"document_public_key":"opaque-string"}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"session.mint","context":{"document_public_key":{"kty":"EC","crv":"P-384","x":"x","y":"y"}}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"session.mint","context":{"document_public_key":{"kty":"EC","crv":"P-256","y":"y"}}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"session.mint","context":{"document_public_key":{"kty":"EC","crv":"P-256","x":"x"}}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"session.prove","context":{"method":"GET","target":"/protected"}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"session.prove","context":{"ticket":TICKET,"target":"/protected"}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"session.prove","context":{"ticket":TICKET,"method":"GET"}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"session.clear","context":{}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"session.clear","context":{"ticket":TICKET,"target":false}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"capability.mint","context":{"action":"profile.module.toggle","target":"homeserver.profile"}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"capability.mint","context":{"ticket":TICKET,"target":"homeserver.profile"}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"capability.mint","context":{"ticket":TICKET,"action":"profile.module.toggle"}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"pin.change","context":{"action":"global.admin.pin.rotate","target":"global.admin.pin"}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"pin.change","context":{"ticket":TICKET,"action":"other","target":"global.admin.pin"}}),
        ),
        (
            "/api/v1/access/challenges/mint",
            serde_json::json!({"purpose":"pin.change","context":{"ticket":TICKET,"action":"global.admin.pin.rotate","target":"other"}}),
        ),
        (
            "/api/v1/access/sessions/mint",
