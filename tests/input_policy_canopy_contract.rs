...
fn input_route_has_exact_snake_seat_and_no_doors_index() {
    ...
    assert_eq!(
        value,
        serde_json::json!({"namespace":"settings/input","admittance":"open","flags":{},"serve":[{"snake":"input-policy"}]})
    );
...
#[test]
fn input_leaf_registers_read_set_and_snake_band() {
    ...
    assert!(source.contains("axum::routing::get(read_http).post(set_http)"));
    assert!(source.contains("crate::gate::snake::run"));
    assert!(source.contains("input-policy"));
